...
def test_normalize_content_name():
    ...
    assert normalize_content_name("Movie.2024.1080p-GROUP [Seedpool]") == "movie.2024.1080p-group"
...
def test_find_by_name_with_bracketed_tags():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            # Query with [Seedpool] variant matches the stored entry
            results = store.find_by_name("Movie.2024.1080p-GROUP [Seedpool]")
            assert len(results) == 1
            assert results[0].source_infohash == "1" * 40
        finally:
            store.close()
