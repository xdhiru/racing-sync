...
class ProwlarrTrackerMap(BaseModel):
    ...
    def _flatten(cls, data: object) -> object:
        """Accept either:
          - a flat dict:        {"alpha.cc": "Alpha (API)"}
        ...
        if isinstance(data, dict):
            ...
            for forbidden in ("overrides", "beta_substr",
                              "beta_index", "alpha_substr",
                              "alpha_index", "gamma_substr",
                              "gamma_index"):
...
class CrossSeedConfig(BaseModel):
    ...
    # under [prowlarr].download_indexer (default "Indexer (API)").
    ...
    @property
    def indexer_retry_interval_seconds(self) -> int:
        return self.prowlarr_retry_interval_seconds

    @property
    def indexer_max_age_seconds(self) -> int:
    ...
    #   content (or the user prefers a Indexer cross-seed), we query
